...
namespace Back_Propagation_NN
{
    class BackPropProgram
    {
        public static void Main(String[] args)
        {
            //Driver function takes 3 arguments : (1)"Train" or "Command", (2) Sensor type, (3) Sensor Data(insert null if you want to train the NN)
            //Run 12 line to train NN, Run 13 line to calculate output from a trained NN
            //String ret_value = Driver("Train", "Radar", null);
            String ret_value = Driver("Command", "Range", new double[] { 4, -1, -1, -1, -1, 4, 1, 0, 1, -1, 4, 4, 2,1,-1,-1,0,0,0,-1,0,-1,0,-1,1 });
        ...
        public static double[][] ExtractTrainData(string sensor_type)
        {
            ...
            for (int i = 0; i < train_rows; i++)
            {
                ...
                for (int j = 0; j < no_of_fields; j++)
                {
                    ...
                    {
                        test[i][j] = 1; test[i][j + 1] = 0; test[i][j + 2] = 0; test[i][j + 3] = 0;
                    }
                    else if ((elements[count].Trim()).Equals("South"))
                    { 
                        test[i][j] = 0; test[i][j + 1] = 1; test[i][j + 2] = 0; test[i][j + 3] = 0;
                    }
                    else if ((elements[count].Trim()).Equals("East"))
                    {
                        test[i][j] = 0; test[i][j + 1] = 0; test[i][j + 2] = 1; test[i][j + 3] = 0;
                    }
                    else if ((elements[count].Trim()).Equals("West"))
                    {
                        test[i][j] = 0; test[i][j + 1] = 0; test[i][j + 2] = 0; test[i][j + 3] = 1;
                    }
                    else
                    {
